...
Definition test_factorial (candidate : nat -> nat) : bool :=
  ((candidate 0) =? 1)
  &&
  ((candidate 1) =? 1 * 1)
  &&
...
Fixpoint factorial_d (n : nat) : nat :=
  match n with
  | O =>
      1
  | S n' =>
...
Compute (test_factorial factorial_d).

Lemma fold_unfold_factorial_d_O :
  factorial_d 0 =
    1.
Proof.
  fold_unfold_tactic factorial_d.
Qed.

Lemma fold_unfold_faactorial_d_S :
  forall n' : nat,
    factorial_d (S n') =
      S n' * factorial_d n'.
Proof.
