...
(* name: Sean Lim
   e-mail address: sean.lim@u.yale-nus.edu.sg
   student number: A0230369E
*)

(* name: Hana Miura
   e-mail address: hanamiura@u.yale-nus.edu.sg
   student number: A0224162R
*)

(* name: Rana Harris Farooq
   e-mail address: ranaharrisfarooq@u.yale-nus.edu.sg
   student number: A0225594X
*)
...
(* Version 3: Not going faster than the music (interludes) *)
...
Fixpoint odd_v2_aux (n : nat) (a : bool) : bool :=
  match n with
  | O => a
  | S p => odd_v2_aux p (negb a)
  end.
...
(*error
Compute (test_number_of_leaves number_of_leaves_acc).
*)
...
Fixpoint number_of_nodes_v1 (t : binary_tree_nat) : nat :=
  match t with
    ...
    0
  | Node_nat t1 t2 =>
    (number_of_nodes_v1 t1) + (number_of_nodes_v1 t2) + 1
  end.
 *)
...
Definition test_smallest_leaf (candidate: binary_tree_nat -> nat) : bool :=
  (Nat.eqb (candidate (Node_nat (Leaf_nat 1)
     ...
     1)
  &&
    (Nat.eqb (candidate (Node_nat (Leaf_nat 3)(Node_nat (Leaf_nat 4) (Leaf_nat 5)))) 3)
...
Fixpoint smallest_leaf_v1 (t : binary_tree_nat) : nat :=
    ...
    n
  | Node_nat t1 t2 =>
      if smallest_leaf_v1 t1 <? smallest_leaf_v1 t2
      then smallest_leaf_v1 t1
      else smallest_leaf_v1 t2
  end.
 *)
...
(*
Fixpoint weight_v1_acc (t: binary_tree_nat) (a: nat):nat :=
  match t with
    Leaf_nat n => S a
  | Node_nat t1 t2 => (weight_v1_acc t1)(weight_v1_acc t2) (S a)
  end.
 *)
...
Definition test_length_of_longest_path (candidate: binary_tree_nat -> nat) : bool :=
  ...
  &&
  (Nat.eqb (candidate (Node_nat (Node_nat (Leaf_nat 1)(Leaf_nat 2)) (Node_nat (Leaf_nat 3) (Leaf_nat 4) )) 2)
  &&
  (Nat.eqb (candidate (Node_nat (Node_nat (Leaf_nat 3) (Leaf_nat 5)) (Node_nat (Leaf_nat 6) (Node_nat (Leaf_nat 4)(Leaf_nat 1))))) 3)
...
Fixpoint length_of_longest_path_v1 (t : binary_tree_nat) : nat :=
    ...
    Leaf_nat n =>
    0
  | Node_nat t1 t2 => 1 +  max (length_of_longest_path_v1 t1)(length_of_longest_path_v1 t2)
  end.
 *)
...
Compute (test_longest_path length_of_longest_path_v1).
...
Definition test_shortest_path (candidate: binary_tree_nat -> nat) : bool :=
  ...
  && (Nat.eqb (candidate (Node_nat (Leaf_nat 3)(Leaf_nat 4)))1)
       && (Nat.eqb (candidate (Node_nat (Node_nat (Node_nat (Leaf_nat 1)(Leaf_nat 2)) (Leaf_nat 3))(Leaf_nat 4)))1)
  (* etc. *)
  .
...
Fixpoint length_of_shortest_path_v1 (t : binary_tree_nat) : nat :=
   match t with
    Leaf_nat n =>
    0
  | Node_nat t1 t2 => 1 + min (length_of_shortest_path_v1 t1)(length_of_shortest_path_v1 t2)
  end.
 *)
...
Compute (test_shortest_path length_of_shortest_path_v1).
...
(*
Definition test_balancedp (balancedp: binary_tree_nat -> bool) : bool :=
...
Fixpoint balancedp_v1 (t : binary_tree_nat) : bool :=
  match t with
    Leaf_nat n => true
  | Node_nat t1 t2 =>
      let difference := Nat.abs(weight_v1 t1 - weight_v1 t2) in balancedp_v1 t1 && balancedp_v2 t2 && difference = 0
  end.
*)
  

(*Compute (test_balancedp balancedp_v1).*)

         

(* Exercise 3: Defining Type*)

Inductive binary_tree_nat' : Type :=
| Leaf_nat' : binary_tree_nat'
| Node_nat': binary_tree_nat' -> nat -> binary_tree_nat' -> binary_tree_nat'.

(* Exercise 3: No. Of Leaves Unit Test*)
...
(* Exercise 3: No. Of Leaves Function*)
...
(* Exercise 3: No. Of Leaves Testing*)

Compute (test_number_of_leaves_ex3 number_of_leaves_ex3_v1).

(* Exercise 3: No. Of Nodes Unit Test*)
...
(* Exercise 3: No. Of Nodes Function*)
...
(* Exercise 4: No. Of Leaves Function*)
...
(* Exercise 4: No. Of Leaves Testing*)

Compute (test_number_of_leaves_ex4 number_of_leaves_ex4_v1).

(* Exercise 4: No. Of Nodes Unit Test*)
...
(* Exercise 4: No. Of Nodes Function*)
...
(* Exercise 4: No. Of Nodes Testing*)
